...
class VolumeGestureTracker:
    ...
    def update(
        ...
        *,
        ...
        trigger_mute = False
        if allow_mute_toggle and stable_gesture == 'mute' and now - self._last_mute_toggle_time >= self.mute_cooldown_seconds:
            self._last_mute_toggle_time = now
            trigger_mute = True
            self._message = 'mute toggle'
            self._status = 'muted' if not current_muted else 'unmuted'

        if features is None or landmarks is None:
            if self._active and now - self._last_seen_time <= self.no_hand_grace_seconds:
