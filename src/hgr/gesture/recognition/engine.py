...
class GestureRecognitionEngine:
    ...
    def _update_stable_label(self, raw_label: str, confidence: float) -> str:
        if raw_label == "neutral":
            self._stable_state = _StableLabelState()
            return "neutral"
        if raw_label == self._stable_state.candidate:
            self._stable_state.count += 1
        else:
            self._stable_state.candidate = raw_label
            self._stable_state.count = 1
        if self._stable_state.count >= self.stable_frames_required and confidence >= 0.56:
            self._stable_state.stable = raw_label
        return self._stable_state.stable
    ...
    def _analyze_tracked_hand(
        ...
        orientation_ready = {
            ...
            "fist": self._gesture_not_inverted(hand_reading, ()),
            "zero": self._gesture_not_inverted(hand_reading, ()),
            "ok": self._gesture_not_inverted(hand_reading, ()),
            "volume_pose": self._gesture_not_inverted(hand_reading, ()),
        ...
        dynamic_label, dynamic_candidates, dynamic_scores = self.dynamic_recognizer.update(hand_reading, timestamp)
        ...
        stable_label = self._update_stable_label(raw_label, raw_confidence)
    ...
    def process_frame(self, frame_bgr: np.ndarray, timestamp: float | None = None) -> GestureFrameResult:
        ...
        return self._analyze_tracked_hand(detection.tracked_hand, detection.frame_bgr, timestamp, frame_index)
