...
def score_swipe_right(context: DynamicDetectionContext) -> dict[str, float]:
    if context.state.sample_count < 4 or not context.state.has_anchor:
        return {}

    s = context.features.open_scores
    primary_open_gate = clamp01((min(s['index'], s['middle']) - 0.52) / 0.20)
    support_open_gate = clamp01((max(s['ring'], s['pinky']) - 0.34) / 0.28)
    ...
    if deltas_x:
        right_consistency = sum(max(0.0, dx) for dx in deltas_x) / max(1e-6, sum(abs(dx) for dx in deltas_x))
        forward_frames = sum(1 for dx in deltas_x if dx > 0.040)
        reverse_frames = sum(1 for dx in deltas_x if dx < -0.030)
    ...
    speed_gate = clamp01((peak_speed - 1.70) / 1.20)
    path_gate = clamp01((path_length - 0.84) / 0.54)
    progress_gate = clamp01((horizontal - 0.58) / 0.28)
    ...
    axis_ratio_gate = clamp01(((horizontal / max(vertical + 0.60 * depth, 1e-6)) - 1.40) / 0.50)
    duration_gate = clamp01((0.55 - duration) / 0.22)
    score = clamp01(
        (
            0.24 * progress_gate
            + 0.18 * path_gate
            + 0.16 * right_consistency
